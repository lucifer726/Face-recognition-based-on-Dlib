...
class RegisterUi(wx.Frame):
    def __init__(self, superion):
        wx.Frame.__init__(self, parent=superion, title="人脸注册", size=(800, 590),
        ...
        self.ShortCutButton = wx.Button(parent=self, pos=(50, 220), size=(80, 50), label='人脸保存')
    ...
    def OnNewButtonClicked(self, event):
        while self.name == '':
            self.name = wx.GetTextFromUser(message="请先输入来访者的姓名", caption="温馨提示",
    ...
    def OnSaveButtonClicked(self, event):
        ...
        if self.register_flag == 0 and self.sc_number != 0:
            ...
            for i in range(len(pics)):
                ...
                dets = face_recognize.detector(img_gray, 1)
                if len(dets) != 0:
                    shape = face_recognize.predictor(img_gray, dets[0])
                    face_descriptor = face_recognize.facerec.compute_face_descriptor(img_gray, shape)
    ...
    def _open_cap(self, event):
        ...
        while self.cap.isOpened():
            ...
            # 人脸数 rects
            self.rects = face_recognize.detector(self.im_rd, 1)
            ...
            if len(self.rects) != 0:
                ...
                # 查重
                features_cap = face_recognize.facerec.compute_face_descriptor(self.im_rd,
                                                                              face_recognize.predictor(self.im_rd,
                                                                                                       self.rects[0]))
                for i in range(len(face_recognize.features_known_arr)):
                    # 将某张人脸与存储的所有人脸数据进行比对
                    compare = face_recognize.return_euclidean_distance(features_cap,
                                                                       face_recognize.features_known_arr[i][0:-1])
                    if compare == "same":  # 找到了相似脸
                        face_name = face_recognize.features_known_arr[i][-1]
                        print(face_name)
                        wx.MessageBox(message=face_name + "，您已录过人脸，请勿重复录入", caption="警告")
            ...
            #cv2.putText(self.im_rd, "out of range", (50, 140), font, 0.8, (0, 0, 255), 1,cv2.LINE_AA)
